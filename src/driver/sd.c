...
static int sdDebugResponse(int resp) {
    printk("- EMMC: Command %s resp %x: %x %x %x %x\n", sdCard.lastCmd->name, resp, *EMMC_RESP3,
           *EMMC_RESP2, *EMMC_RESP1, *EMMC_RESP0);
...
static int sdSendCommandP(EMMCCommand* cmd, int arg) {
    ...
    *EMMC_CMDTM = cmd->code;
    ...
    case RESP_R6:
        ...
        sdCard.status = (u32)((resp0 & 0x00001fff))  // 12:0 map directly to status 12:0
...
static void sdParseCID() {
    ...
    int serial
        = (int)(((sdCard.cid[2] & 0x0000ffff) << 16) + ((sdCard.cid[3] & 0xffff0000) >> 16));
    ...
    printk("- EMMC: SD Card %s %dMb UHS-I %d mfr %d '%s:%s' r%d.%d %d/%d, #%x RCA "
           "%x\n",
           SD_TYPE_NAME[sdCard.type], (int)(sdCard.capacity >> 20), sdCard.uhsi, manId, appId,
           name, revH, revL, dateM, dateY, serial, sdCard.rca >> 16);
}
